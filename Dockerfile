...
# (We could rely on the list in geordi.cabal, but having these here shortens the iteration cycle when I'm testing changes in geordi.)
...
#RUN dpkg --force-all -r grep findutils
RUN rm -rf /home /mnt /opt /var /sbin /srv /boot /media /root /usr/sbin /bin
