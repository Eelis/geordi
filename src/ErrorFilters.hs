{-# LANGUAGE UnicodeSyntax, MultiParamTypeClasses, FunctionalDependencies, FlexibleInstances, TypeSynonymInstances, UndecidableInstances, ViewPatterns, FlexibleContexts #-}
...
hide_clutter_namespaces = subRegex' (mkRegex "(\\b|:: *)(std|__(gnu_)?(debug(_def)?|norm|cxx))::") ""
...
instance Parser String () String where parser = string
instance Parser [String] () String where parser = choice . ((try . string) .)
instance Parser Char () Char where parser = char
...
-}
  ...
  tmpi :: String → Int → CharParser () [String]
